...
The PTY recipe sets the host pixel dimensions explicitly. It decodes six pane strips, two sidebar strips when visible, and four named-pane strips. It checks their complete pixel arrays and confirms that streaming text reuses uploads. The named run checks a real CJK label with a border glyph at literal cell positions. Replay `named-panes/raw.ansi` in Ghostty for the native proof, then compare the chip fill and text against the PTY's decoded pixels. Readiness must use the last completed synchronized-output frame (`CSI ? 2026 l`): a pane marker can arrive while the border or graphics bytes are still being written.
