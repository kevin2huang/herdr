...
`sidebar_bg` を使うと、デスクトップのサイドバーだけに背景色を設定できます。省略した場合、サイドバーはホストターミナルの背景を使います。`active_row_bg` は、区切り線やスクロールバートラックに影響を与えず、アクティブな Space とフォーカス中の Agent 行の背景色を変更します。`selection_bg` は、サイドバーの Navigate モードのカーソル行の背景色を変更します。`pane_gap_bg` は、ペイン間の隙間やリサイズ中に一時的に生じる余白など、Herdr が所有するペインキャンバスのセルを変更します。`ui.pane_gaps = true` の場合、枠付きペインを左右に並べると 1 列の隙間が入り、上下に並べると空行は入りません。枠なしペインの間には両方向とも 1 セル幅の隙間が確保されます。`pane_default_bg` は、ターミナルプログラムが明示的に指定した背景色を保ったまま、ターミナルコンテンツ、境界とスクロールバー用の列、ポップアップターミナルのデフォルト背景セルを変更します。この隙間はターミナルコンテンツ内の余分なパディングではありません。
