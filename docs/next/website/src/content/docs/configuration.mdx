...
`sidebar_bg` optionally gives the desktop sidebar its own background. When omitted, the sidebar keeps the host terminal background. `active_row_bg` changes the active Space and focused Agent row background without affecting separators or scrollbar tracks. `selection_bg` changes the Navigate-mode cursor row background in the sidebar. `pane_gap_bg` changes Herdr-owned pane canvas cells, including exterior gaps and temporary resize slack. When `ui.pane_gaps = true`, bordered panes have one blank column between side-by-side frames and no blank row between stacked frames. Borderless panes reserve a one-cell gap in both directions. `pane_default_bg` changes default-background cells in terminal content, border and scrollbar lanes, and popup terminals while preserving explicit backgrounds drawn by terminal programs. The pane gap is not padding inside terminal content.
