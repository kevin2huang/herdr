...
`sidebar_bg` 可单独设置桌面侧边栏的背景色。省略时，侧边栏继续使用宿主终端背景。`active_row_bg` 可更改当前 Space 和已聚焦 Agent 行的背景色，而不会影响分隔线或滚动条轨道。`selection_bg` 可更改侧边栏中 Navigate 模式光标行的背景色。`pane_gap_bg` 可更改 Herdr 所有的窗格画布单元格，包括窗格间隙以及调整大小时暂时出现的空白。设置 `ui.pane_gaps = true` 后，左右排列的带边框窗格之间保留一列空白，上下排列时不保留空行。无边框窗格在两个方向上都保留一格宽的间隙。`pane_default_bg` 可更改终端内容、边框和滚动条区域以及弹出终端中的默认背景单元格，并保留终端程序明确绘制的背景色。这段间隙不是终端内容内的额外内边距。
